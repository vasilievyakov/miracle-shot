...
![Скроллинг-захват: приложение само листает окно](docs/media/scroll.gif)

Результат этого захвата, страница 3116 на 13368 пикселей, разрезанная на три колонки, чтобы поместиться здесь:

![Склеенная страница в три колонки](docs/media/scroll-result.png)
...
![Панель превью](docs/media/preview.png)
...
![Редактор со стрелкой, эллипсом и прямоугольником](docs/media/editor.png)
...
![Закрепленный снимок остается на месте, пока страница листается под ним](docs/media/pin.gif)
...
![Пресет Lab Brand вокруг снимка](docs/media/background.png)
...
Цифры на момент написания: около 7 000 строк кода, 4 500 строк тестов, 411 тестов, 160+ коммитов.
...
- Склейщик писался для страниц, которые листают вниз. В реальных захватах пользователь листал в обе стороны, терминальный UI прыгал на страницу от первого события колеса, полупрозрачная рамка Finder менялась на несколько уровней вместе с тем, что за окном, плашка «jump to bottom» висела поверх контента, а список файлов с разреженным текстом на полосатых строках почти совпадал при сдвиге на целую строку. Потом сайт самой лаборатории добавил секцию с плашками, которые плавают вечно: ни одно совмещение не бывает точным, а пустые поля между секциями совпадают лучше, чем контент. Теперь матчер берет то совмещение, чьи совпавшие строки объясняют больше всего деталей, а не самое близкое, пробует все четыре способа, которыми кадр может относиться к странице, выводит закрепленные элементы из сравнения, судит о статичных краях по медиане пар, ждет reveal-анимации по полосам, а захват подстраивает шаг прокрутки под реально измеренный сдвиг. За каждым из этих правил стоит синтетический тест и реальный захват.
